...
pub struct TypeScriptOptions {
    ...
    /// This is so that we know that the import is not a type import, and should
    /// not be removed. defaults to React
    ...
    /// This is so that we know that the import is not a type import, and should
    /// not be removed. defaults to React.Fragment
    ...
    /// (introduced in TypeScript 3.8). This should only be used if you are
    /// using TypeScript >= 3.8.
    ...
    /// ## Deprecated
    ///
    /// Allowing `declare` fields is built-in support in SWC without any option.
    /// If you want to remove class fields without initializer, you can use
    /// `remove_class_fields_without_initializer: true` instead.
    #[serde(default = "default_as_true")]
    ...
    /// ```
    ///
    /// The option is used to align with the behavior of TypeScript's
    /// `useDefineForClassFields: false` option. When you want to enable
    /// this, you also need to set
    /// [`crate::compat::compiler_assumptions::Assumptions::set_public_class_fields`]
    /// to `true`. The `set_public_class_fields: true` +
    /// `remove_class_fields_without_initializer: true` is equivalent to
    /// `useDefineForClassFields: false` in TypeScript.
    ///
    /// When `set_public_class_fields` is true and class-properties plugin is
    /// enabled, the above example transforms into:
    ///
    /// ```js
    /// class Foo {
    ///   constructor() {
    ...
    /// allows writing complete relative specifiers in import declarations while
    /// using the same extension used by the source files.
    ///
    /// When set to `true`, same as [`RewriteExtensionsMode::Rewrite`]. Defaults
    /// to `false` (do nothing).
...
pub enum RewriteExtensionsMode {
    /// Rewrite `.ts`/`.mts`/`.cts` extensions in import/export declarations to
    /// `.js`/`.mjs`/`.cjs`.
    #[default]
    Rewrite,
    /// Remove `.ts`/`.mts`/`.cts`/`.tsx` extensions in import/export
    /// declarations.
    Remove,
}
            ...
            E: de::Error,
        {
            match value {
                ...
                _ => Err(E::custom(format!(
                    "Expected RewriteExtensionsMode is either \"rewrite\" or \"remove\" but \
                     found: {value}"
                ))),
            }
        }
